...
def draw_top_down_map(
        ...
        reference_line_color=(180, 180, 180),
    ...
    if isinstance(map, ScenarioMap):
        ...
        if reference_line is not None:
            if not isinstance(reference_line, InterpolatingLine):
                reference_line = InterpolatingLine(reference_line)
            if reference_line_width is not None:
                reference_line.__setattr__("width", reference_line_width)
            LaneGraphics.draw_drivable_area(reference_line, surface, color=reference_line_color)
